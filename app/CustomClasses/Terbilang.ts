const IDreadThousand = function (n, dg, snum) {
  var s = ''
...
const readAll = function (x) {
  ...
  if (isfailed == false) {
    do {
      var groupNumbers = Math.round(
        (Math.floor(x) / 1000 - Math.floor(Math.floor(x) / 1000)) * 1000
      )
      s = IDreadThousand(groupNumbers, i, snum) + thousandDesc[i] + ' ' + s
      if (x == 0) {
        s = 'nol'
      }
