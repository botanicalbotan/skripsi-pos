...
export default class PegawaisController {
  public async index({ view, request, auth, response }: HttpContextContract) {
    ...
    const pegawais = await Database.from('penggunas')
      ...
      .select(
        ...
        'penggunas.gender as gender',
        'jabatans.nama as jabatan',
        'penggunas.apakah_pegawai_aktif as apakahAktif',
        'penggunas.gaji_bulanan as gajiBulanan',
        'penggunas.foto as foto',
        'penggunas.tanggal_gajian_selanjutnya as gajiSelanjutnya'
      )
      .if(cari !== '', (query) => {
        query.where('penggunas.nama', 'like', `%${cari}%`)
      })
      .andWhere('penggunas.super', false) // ngga bisa ngambil super user
  ...
  public async store({ request, response, session }: HttpContextContract) {
    const newPegawaiSchema = schema.create({
      jabatan: schema.enum(['karyawan', 'kepalatoko', 'pemiliktoko'] as const),
      nama: schema.string({ trim: true }, [rules.maxLength(50)]),
      gender: schema.enum(['L', 'P'] as const),
      tempatLahir: schema.string({ trim: true }, [rules.maxLength(100)]),
      tanggalLahir: schema.date(),
      alamat: schema.string({ trim: true }, [rules.maxLength(100)]),
      catatan: schema.string.optional({ trim: true }, [rules.maxLength(100)]),
      ...
      status: schema.enum(['aktif', 'keluar'] as const),
      tanggalMulaiAktif: schema.date.optional({}, [rules.requiredWhen('status', '=', 'aktif')]),
      gajiBulanan: schema.number([rules.unsigned()]),
    ...
    try {
      ...
      namaFileFoto =
        (validrequest.nama || '').replace(/\s/gm, '') + DateTime.now().toMillis() + '.jpg'
    ...
    try {
      ...
      const penggunabaru = await userbaru.related('pengguna').create({
        ...
        tanggalMulaiAktif:
          validrequest.status === 'aktif' && validrequest.tanggalMulaiAktif ? DateTime.now() : null,
        tanggalGajianSelanjutnya:
          validrequest.status === 'aktif' ? DateTime.now().plus({ months: 1 }) : null,
        jabatanId: idJabatan,
      })
      ...
      session.flash(
        'alertError',
        'Ada masalah saat membuat pegawai baru. Silahkan coba lagi setelah beberapa saat.'
      )
  ...
  public async show({ view, params, response, session, auth }: HttpContextContract) {
    try {
      ...
      await pegawai.load('penerimaGajis')
      ...
      let gaji = 0
      let jumlah = 0
      pegawai.penerimaGajis.forEach((element) => {
        if (!element.dibayarAt && !element.pencatatGajianId) {
          gaji += element.nominalGaji
          jumlah++
        }
      })
      ...
      let tambahan = {
        ...
        bisaEdit:
          userPengakses.pengguna.id === pegawai.id ||
  ...
  public async update({ request, response, session, params, auth }: HttpContextContract) {
    const editPegawaiSchema = schema.create({
      jabatan: schema.enum.optional(['karyawan', 'kepalatoko', 'pemiliktoko'] as const),
      nama: schema.string({ trim: true }, [rules.maxLength(50)]),
      gender: schema.enum(['L', 'P'] as const),
      tempatLahir: schema.string({ trim: true }, [rules.maxLength(100)]),
      tanggalLahir: schema.date(),
    ...
    try {
      ...
      pengguna.tanggalMulaiAktif =
        validrequest.status === 'aktif' && validrequest.tanggalMulaiAktif
          ? validrequest.tanggalMulaiAktif
          : null
      pengguna.tanggalGajianSelanjutnya =
        validrequest.status === 'aktif' && validrequest.tanggalGajianSelanjutnya
          ? validrequest.tanggalGajianSelanjutnya
          : null
      ...
      session.flash(
        'alertError',
        'Ada masalah saat mengubah data pegawai. Silahkan coba lagi setelah beberapa saat.'
      )
      return response.redirect().back()
    }
  }
  ...
  public async destroy({ params, response, session }: HttpContextContract) {
    try {
      ...
      session.flash(
        'alertError',
        'Ada masalah saat menghapus data pegawai. Silahkan coba lagi setelah beberapa saat.'
      )
      return response.redirect().back()
    }
  }

  public async showListTagihan({ params, view, response, session, request }: HttpContextContract) {
    try {
      const pegawai = await Pengguna.findOrFail(params.id)
      // await pegawai.load('penerimaGajis')
      await pegawai.load('jabatan')

      const opsiOrder = ['tanggalJatuhTempo', 'nominalGaji', 'status', 'dibayarAt', 'namaPencatat']
      const page = request.input('page', 1)
      const order = request.input('ob', 0)
      const sanitizedOrder = order < opsiOrder.length && order >= 0 && order ? order : 0
      const arahOrder = request.input('aob', 0)
      ...
      const tagihans = await Database.from('penggajian_pegawais')
        .leftJoin('penggunas', 'penggunas.id', 'penggajian_pegawais.pencatat_gajian_id')
        .select(
          'penggajian_pegawais.status as status',
          'penggajian_pegawais.tanggal_seharusnya_dibayar as tanggalJatuhTempo',
          'penggajian_pegawais.dibayar_at as dibayarAt',
          'penggajian_pegawais.nominal_gaji as nominalGaji',
          'penggunas.nama as namaPencatat',
          'penggunas.id as idPencatat'
        )
        .where('penggajian_pegawais.penerima_gaji_id', pegawai.id)
        .if(sanitizedFilterShow == 1, (query) => {
          query
            .whereNull('penggajian_pegawais.dibayar_at')
        })
        .orderBy(opsiOrder[sanitizedOrder], sanitizedArahOrder == 1 ? 'desc' : 'asc')
        .orderBy('tanggalJatuhTempo')
        .paginate(page, limit)

      tagihans.baseUrl('/app/pegawai/' + params.id + '/tagihan-gaji')
      ...
      if (sanitizedFilterShow !== 0) qsParam['fs'] = sanitizedFilterShow
      tagihans.queryString(qsParam)

      let firstPage =
        tagihans.currentPage - 2 > 0
          ? tagihans.currentPage - 2
          : tagihans.currentPage - 1 > 0
          ? tagihans.currentPage - 1
          : tagihans.currentPage
      let lastPage =
        tagihans.currentPage + 2 <= tagihans.lastPage
          ? tagihans.currentPage + 2
          : tagihans.currentPage + 1 <= tagihans.lastPage
          ? tagihans.currentPage + 1
          : tagihans.currentPage

      if (lastPage - firstPage < 4 && tagihans.lastPage > 4) {
        if (tagihans.currentPage < tagihans.firstPage + 2) {
          lastPage += 4 - (lastPage - firstPage)
        }

        if (lastPage == tagihans.lastPage) {
          firstPage -= 4 - (lastPage - firstPage)
        }
      }
      // sampe sini
      const tempLastData = 10 + (tagihans.currentPage - 1) * limit
      ...
      const tambahan = {
        ...
        firstPage: firstPage,
        lastPage: lastPage,
        firstDataInPage: 1 + (tagihans.currentPage - 1) * limit,
        lastDataInPage: tempLastData >= tagihans.total ? tagihans.total : tempLastData,
      }
  ...
  public async ubahUsername({ response, request, params, auth }: HttpContextContract) {
    ...
    try {
      ...
      if (cekPegawai && cekPegawai.id !== pegawai.user.id)
        throw 'Username sudah digunakan. Silahkan gunakan username yang lain!'
  ...
  public async ubahEmail({ request, auth, params, response }: HttpContextContract) {
    const newEmailSchema = schema.create({
      newEmail: schema.string({ trim: true }, [rules.email()]),
    })

    try {
      if (!auth.user) throw 'ngga valid'

      const validrequest = await request.validate({ schema: newEmailSchema })
      ...
      const cekPegawai = await User.findBy('email', validrequest.newEmail)
      if (cekPegawai && cekPegawai.id !== pegawai.user.id)
        throw 'Email sudah digunakan. Silahkan gunakan email yang lain!'

      pegawai.user.email = validrequest.newEmail
      await pegawai.user.save()

      return response.ok({ message: 'Email berhasil diubah' })
    } catch (error) {
      if (typeof error === 'string') {
        return response.badRequest({ error: error })
      } else {
