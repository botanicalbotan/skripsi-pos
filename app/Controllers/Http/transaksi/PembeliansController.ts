...
export default class PembeliansController {
  ...
  public async simpanTransaksi({ request, response, session, auth }: HttpContextContract) {
    const newPembelianSchema = schema.create({
      ...
      namaToko: schema.string.optional(
        ...
        [rules.maxLength(50), rules.requiredWhen('kelengkapanNota', '=', 'dengan')]
      ), // boleh null
      alamatToko: schema.string.optional(
        {
          trim: true,
        },
    ...
    try {
      ...
      // --------------- cek nota leo --------------------- OK
      if (
        validrequest.kelengkapanNota == 'dengan' &&
        validrequest.asalPerhiasan == 'leo' &&
        validrequest.hargaJualNota &&
        validrequest.potonganNota &&
        validrequest.tanggalBeli
      ) {
        adaNotaLeo = true
      }
      ...
      if (validrequest.tanggalBeli && apakahJanjiTT) {
        let target = validrequest.tanggalBeli
          .plus({
            day: 1,
          })
          .set({
      ...
      if (apakahRosok) {
        ...
        if (apakahTT) {
          ...
          hargaPerGram =
            (hargaMal *
              (kodepro.kadar.persentaseMalRosok - kodepro.kadar.marginPersenUntungRosokTtMax)) /
            100
          hargaBeliTarget = hargaPerGram * validrequest.beratBarang

          hargaBeliMaksDitawar =
            ((hargaMal *
              (kodepro.kadar.persentaseMalRosok - kodepro.kadar.marginPersenUntungRosokTtMin)) /
              100) *
            validrequest.beratBarang
        } else {
          // --------------- Rumus B, Jual Rosok ---------------------
          rumusDipake = 'B'
        ...
        if (adaNotaLeo) {
            ...
            if (apakahJanjiTT) {
              if (apakahTerlambatTT) {
                if (apakahBarangDipakai) {
                  ...
                  let penguranganPotongan = apakahPelangganTetap
                    ? kodepro.kadar.toleransiPenguranganPotonganMax
                    : kodepro.kadar.toleransiPenguranganPotonganMin
                  let penguranganPotonganMaxTawar = kodepro.kadar.toleransiPenguranganPotonganMax
                  let penguranganPotonganDeskripsi = kodepro.kadar.apakahPotonganPersen
                    ? `${penguranganPotongan}%`
                    : `${rupiahParser(penguranganPotongan)} per gram`
                  ...
                  potongan = kodepro.kadar.apakahPotonganPersen
                    ? (potonganBaru / 100) * hargaJualNota
                    : hitungPotonganNormal(potonganBaru, BERATVALID)
                  let potonganTawar = kodepro.kadar.apakahPotonganPersen
                    ? (potonganBaruTawar / 100) * hargaJualNota
                    : hitungPotonganNormal(potonganBaruTawar, BERATVALID)
                  ...
                  penalti =
                    BERATVALID >= 5
                      ? pengaturan.penaltiTelatJanjiMax
                      : pengaturan.penaltiTelatJanjiMin
                  keteranganTransaksi = `Potongan diganti penalti keterlambatan ${rupiahParser(
                    penalti
                  )} `
                  potongan = 0
          ...
          if (apakahTT) {
            ...
            hargaPerGram =
              (hargaMal *
                (kodepro.kadar.persentaseMalUripan - kodepro.kadar.marginPersenUntungUripanTtMax)) /
              100

            hargaBeliTarget = hargaPerGram * validrequest.beratBarang
            hargaBeliMaksDitawar =
              ((hargaMal *
                (kodepro.kadar.persentaseMalUripan - kodepro.kadar.marginPersenUntungUripanTtMin)) /
                100) *
              validrequest.beratBarang
          } else {
      ...
      let namaBarang = `${apakahTT ? 'TT' : 'JU'} 1 ${model.bentuk.kode} ${
        kodepro.kadar.nama
      } model ${model.nama} ${BERATVALID} gr ${kondisiBarang.toUpperCase()} ${
        adaNotaLeo ? 'Toko LEO ' : 'toko luar'
      }`
      ...
      if (adaNotaLeo) {
        ...
        validrequest.kelengkapanNota === 'dengan' &&
        validrequest.namaToko &&
        validrequest.alamatToko
      ) {
        namaToko = validrequest.namaToko
        alamatToko = validrequest.alamatToko
      }
      ...
      return response
        .redirect()
        .withQs({ tid: pembelianBaru.id })
        .toPath('/app/transaksi/pembelian/pasca')
    } catch (error) {
      session.flash(
        'alertError',
        'Ada kesalahan saat menghitung rumus, silahkan coba lagi setelah beberapa saat!'
      )
  ...
  public async destroy({params, auth, response, session}: HttpContextContract) {
    try {
      if(!auth.user) throw 'Authentikasi tidak valid!'
      const userPengakses = await User.findOrFail(auth.user.id)
      await userPengakses.load('pengguna')
      ...
      await cariRH.related('kas').create({
        ...
        perihal: 'Pembatalan pembelian "' + PB.namaBarang + '" yang tercatat pada tanggal ' + PB.createdAt.toFormat('dd LLL yyyy'),
        nominal: Math.abs(PB.hargaBeliAkhir),
        penggunaId: userPengakses.pengguna.id
      })

      pengaturan.saldoToko += Math.abs(PB.hargaBeliAkhir)
      await pengaturan.save()

      PB.deletedAt = DateTime.now()
      await PB.save()

      session.flash('alertSukses', 'Transaksi pembelian "'+ PB.namaBarang +'" berhasil dihapus!')
      return response.redirect().toPath('/app/riwayat/pembelian')
    } catch (error) {
    ...
    try {
      ...
      if (apakahRosok) {
        ...
        if (adaNotaLeo) {
          ...
          // khusus kalau ada nota LEO, ngitung apa2 pake berat NOTA, bukan berat BARANG
          let BERATVALID = validrequest.beratNota
            ? validrequest.beratNota
            : validrequest.beratBarang
              ...
              if (apakahTerlambatTT) {
                if (apakahBarangDipakai) {
                  ...
                  potonganDeskripsi = kodepro.kadar.apakahPotonganPersen
                    ? `${potonganBaru}% harga jual`
                    : `${rupiahParser(potonganBaru)}/g`
                  potongan = kodepro.kadar.apakahPotonganPersen
                    ? (potonganBaru / 100) * hargaJualNota
                    : hitungPotonganNormal(potonganBaru, BERATVALID)
      ...
      if (adaNotaLeo && validrequest.potonganNota) {
        potonganDeskripsiNota = kodepro.kadar.apakahPotonganPersen
          ? `${validrequest.potonganNota}% harga jual`
          : `${rupiahParser(validrequest.potonganNota)}/g`
      }
      ...
      return {
        ...
        dataNota: {
          ...
          hargaJualNota:
            adaNotaLeo && validrequest.hargaJualNota
              ? rupiahParser(validrequest.hargaJualNota)
              : null, // OK
